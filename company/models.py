...
def hero_directory_path(instance, filename):
    ...
    if settings.USE_CLOUDINARY:
        # Cloudinary-style path
        return f'hero/{hero_image_name}{ext}'
    else:
        # Local path inside MEDIA_ROOT
        return os.path.join('hero', hero_image_name, f'{hero_image_name}{ext}')
...
def about_directory_path(instance, filename):
    ...
    if settings.USE_CLOUDINARY:
        return f'about/{about_image_name}{ext}'
    else:
        return os.path.join('about', about_image_name, f'{about_image_name}{ext}')
...
def event_directory_path(instance, filename):
    ...
    if settings.USE_CLOUDINARY:
        return f'events/{event_name}/{event_name}{ext}'
    else:
        return os.path.join('events', event_name, f'{event_name}{ext}')
...
def event_files_directory_path(instance, filename):
    ...
    if instance.caption:
        filename = f"{instance.caption}.{get_ext}"
    else:
        filename = filename

    if instance.media_type == 'photo':
        return os.path.join('events_media', str(event_name), 'images', filename)
    elif instance.media_type == 'video':
        return os.path.join('events_media', str(event_name), 'videos', filename)


# === CLOUDINARY UPLOAD PATH (for public_id) ===
def event_files_directory_path_cloudinary(instance):
    event_name = instance.event
    ext = '.png' if instance.media_type == 'photo' else ''

    if instance.media_type == 'photo':
        return f"events_media/{event_name}/images/{instance.caption or 'untitled'}{ext}"
    elif instance.media_type == 'video':
        return f"events_media/{event_name}/videos/{instance.caption or 'untitled'}"
...
def create_video_thumbnail(instance):
    if instance.media_type == 'video':
        ...
        with open(thumbnail_path, 'rb') as f:
            ...
            final_thumbnail_path = os.path.join(thumbnail_dir, f'{instance.id}_thumbnail.jpg')
...
class Event_Media(models.Model):
    ...
    # Auto-switch field type
    if settings.USE_CLOUDINARY:
        file = CloudinaryField(
            'media',
            folder=event_files_directory_path_cloudinary,
            blank=True,
            null=True,
        ...
        file = models.FileField(
            upload_to=event_files_directory_path,
            blank=True,
            null=True,
            default='events/event_1.jpg'
        )
        # Local only: generate thumbnail from local file
        media_thumbnail = ImageSpecField(
            source='file',
            processors=[ResizeToFill(80, 70)],
            format='JPEG',
            options={'quality': 60}
        )
    ...
    def get_thumbnail_url(self, height=150, width=150):
        """Returns a thumbnail URL depending on environment."""
        if settings.USE_CLOUDINARY and self.file:
            # Cloudinary transforms image on the fly
            return self.file.build_url(height=height, width=width, crop='fill')
        elif not settings.USE_CLOUDINARY and hasattr(self, 'media_thumbnail') and self.media_thumbnail:
            # Local imagekit-generated thumbnail
